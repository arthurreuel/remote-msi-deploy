...
| Segredos no pacote portátil | `Empacotar.ps1` avisa que o `.zip` inclui o `token.txt`; trate-o como confidencial. |
...
4. **Token**: o `token.txt` é um segredo. Fica protegido pela ACL da pasta; não versione, e cuidado ao gerar/transportar o pacote portátil.
